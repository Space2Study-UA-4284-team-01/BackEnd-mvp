...
describe('Subject Service', () => {
  ...
  test('should throw 404 CATEGORY_NOT_FOUND when category does not exist in the database', async () => {
    ...
    // Subject lookup should never be reached when category is missing
    expect(Subject.findOne).not.toHaveBeenCalled()
  })

  test('should throw 422 when category is not a valid ObjectId', async () => {
    const invalidData = { ...validSubjectData, category: 'not-a-valid-objectid' }

    await expect(subjectService.createSubject(invalidData)).rejects.toMatchObject({
      status: 422
    })
    // Category DB lookup should not be reached for an invalid ObjectId
    expect(Category.findById).not.toHaveBeenCalled()
  })

  test('should call validateFunc.type with name field and string type', async () => {
    Subject.findOne.mockResolvedValue(null)
    Subject.create.mockResolvedValue({ ...validSubjectData })

    await subjectService.createSubject(validSubjectData)

    expect(validateFunc.type).toHaveBeenCalledWith('name', 'string', validSubjectData.name)
  })

  test('should call Subject.create with normalized (trimmed) name', async () => {
    const dataWithSpaces = { ...validSubjectData, name: '  Mathematics  ' }
    Subject.findOne.mockResolvedValue(null)
    Subject.create.mockResolvedValue({ ...validSubjectData })

    await subjectService.createSubject(dataWithSpaces)
